...
def list_objects_generator(pickled_triples):
	"""
	:param pickled_triples: pickled triples of (object, relation, country) from the Union of Concerned Scientists datset
	:return: List[str] , str is name of ASO
	"""

	triple_counter = Counter()
	for triple in triples:
		triple_counter.update({triple[0]})
	objects_list = list(triple_counter.keys())
	return objects_list


def enclosed_sentences(file_list, objects_list,nlp,num_intervals, target_dir):
	"""
	:param file_list: List[str] of file names within text corpus
	:param objects_list: List[str] , str is name of ASO
	:param nlp: Hugging Face pipeline object, contains tokenizer, NER model, GPU acceleration done automatically
	:param num_intervals: int, number of checkpoints in case kicked out of HPC node
	:param target_dir: str, 
	:return: List[str] , enclosed sentences containing ASO mentions
	"""
	start = time.time()
...
def file_list_gen(path_name):
	"""
	:param path_name: str, path directory with corpus
	:return: List[str] , str is name of each individual .txt file. For other functions
	"""
	os.chdir(path_name)
	file_list = []
